...
private:
    int **pa;     // вказівник на майбутній масив
...
UnsignedIntArray operator+(const UnsignedIntArray &a, const UnsignedIntArray &b)
{
    ...
    UnsignedIntArray result(a.rows, a.cols);
    for (int i = 0; i < a.rows; i++)
    {
        for (int j = 0; j < a.cols; j++)
        {
            int s1 = a.pa[i][j];
            int s2 = b.pa[i][j];
            int sum = a.pa[i][j] + b.pa[i][j];

            result.pa[i][j] = a.pa[i][j] + b.pa[i][j];
        }
    }
    return result;
}
...
UnsignedIntArray operator*(const UnsignedIntArray &a, const UnsignedIntArray &b)
{
    if (a.rows != b.cols)
    {
        throw std::invalid_argument("The number of columns of the first matrix is equal to the number of rows of the second matrix.");
    }

    UnsignedIntArray result(a.rows, b.cols);
    for (int i = 0; i < a.rows; i++)
    {
        for (int j = 0; j < b.cols; j++)
        {
            ...
            {
                result.pa[i][j] += b.pa[i][k] * a.pa[k][j];
            }
        }
    }
    return result;
